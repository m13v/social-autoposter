...
1. Find candidate threads (use the helper script: python3 $REPO_DIR/scripts/find_threads.py --include-moltbook --include-twitter --include-linkedin --force). The script handles rate limiting. For Twitter/LinkedIn candidates (discovery_method: search_url), browse the search URL via the platform's dedicated agent to find actual threads.
