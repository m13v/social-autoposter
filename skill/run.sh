...
# Generate top performers feedback report
TOP_REPORT=$(python3 "$REPO_DIR/scripts/top_performers.py" 2>/dev/null || echo "(top performers report unavailable)")
...
CRITICAL: If a browser agent tool call is blocked or times out, DO NOT fall back to any other browser tool. Wait 30 seconds and retry the same agent. Repeat up to 3 times. If still blocked, skip that platform and move on." 2>&1 | tee -a "$LOG_FILE"
