...
# Platform lock: wait up to 45min for any previous dm-outreach-twitter run, then skip
...
gtimeout 2700 claude --strict-mcp-config --mcp-config "$HOME/.claude/browser-agent-configs/twitter-agent-mcp.json" -p "$(cat "$PROMPT_FILE")" 2>&1 | tee -a "$LOG_FILE" || log "WARNING: Twitter DM outreach claude exited with code $?"
rm -f "$PROMPT_FILE"
