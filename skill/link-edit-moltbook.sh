...
gtimeout 1800 "$REPO_DIR/scripts/run_claude.sh" "link-edit-moltbook" -p "$(cat "$PROMPT_FILE")" 2>&1 | tee -a "$LOG_FILE" || log "WARNING: Moltbook link-edit claude exited with code $?"
rm -f "$PROMPT_FILE"
