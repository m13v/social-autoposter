...
9. If a post is SKIPPED (no project match, comment not found, removed, bad URL), ALWAYS mark it so it won't be retried:
   psql "\$DATABASE_URL" -c "UPDATE posts SET link_edited_at=NOW(), link_edit_content='SKIPPED: REASON' WHERE id=POST_ID"
PROMPT_EOF

gtimeout 1800 "$REPO_DIR/scripts/run_claude.sh" "link-edit-moltbook" --strict-mcp-config --mcp-config "$HOME/.claude/browser-agent-configs/no-agents-mcp.json" -p "$(cat "$PROMPT_FILE")" 2>&1 | tee -a "$LOG_FILE" || log "WARNING: Moltbook link-edit claude exited with code $?"
rm -f "$PROMPT_FILE"
...
python3 "$REPO_DIR/scripts/log_run.py" --script "link_edit_moltbook" --posted 0 --skipped 0 --failed 0 --cost 0 --elapsed "$RUN_ELAPSED"
