...
for line in ps.splitlines():
    ...
    try:
        ...
        continue
    if any(p.get('url', '').startswith('https://x.com/home') for p in pages):
        print(port)
        raise SystemExit(0)
raise SystemExit('No live X home browser agent found')
PY
)"

python3 "$REPO_DIR/scripts/export_cdp_storage_state.py" \
  --port "$PORT" \
  --page-url-prefix "https://x.com/home" \
  --cookie-url "https://x.com/home" \
  --cookie-url "https://x.com/" \
  --cookie-url "https://twitter.com/" \
  --domain-filter "x.com" \
  --merge \
  --backup \
  --require-cookie "auth_token" \
  --out "$OUT"
